      ...
      {coachTeamPending ? (
        ...
        <>
          ...
          {userDetails.role_id === 2 ? (
            <View style={styles(colors.background).buttonView}>
              <TouchableOpacity
                style={styles().button}
                onPress={() => {
                  navigateTo('NewPlayer');
                }}
              >
                <Text style={{ color: '#fff' }}>Add Player</Text>
              </TouchableOpacity>
              {coachData?.team_player.length !== 0 ? (
                <TouchableOpacity
                  style={styles().button}
                  onPress={() => setDeletePopup(true)}
                  disabled={playerId ? false : true}
                >
                  <Text style={{ color: playerId ? '#fff' : '#5b829c' }}>
                    {removePlayersPending ? (
      ...
      {deletePopup ? (
        ...
        >
          <View style={styles().centered_view2}>
            <View style={styles(colors.background).deleteInnerView2}>
              <View style={styles().warningTitle}>
                <Text style={styles(colors.dateTextHome).deletePopupTitleText}>
                  Are you sure you want to remove player from the team?
                </Text>
              </View>
