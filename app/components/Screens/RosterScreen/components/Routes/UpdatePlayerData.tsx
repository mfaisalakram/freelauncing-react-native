...
import { getGetPlayerSelector } from '../../../../../store/getPlayers/selector';
...
export function UpdatePlayerData() {
  ...
  const [userName, setUserName] = useState(profile[0].user.first_name);
  const [weight, setWeight] = useState(profile[0].user_team_detail.weight);
  const [height, setHeight] = useState(profile[0].user_team_detail.height);
  const [userEmail, setUserEmail] = useState(profile[0].user.email);
  const [userLastName, setUserLastName] = useState(profile[0].user.last_name);
  const [userContact, setUserContact] = useState(profile[0].user.phone_number);
  const [userJersey, setUserJersey] = useState(
    profile[0].user_team_detail.jersey
  );
  const [dob, setDob] = useState(profile[0].user_team_detail.birthdate);
  const [HomeTown, setHomeTown] = useState(
    profile[0].user_team_detail.hometown
  );
  const [userPosition, setUserPosition] = useState(
    profile[0].user_team_detail.position
  );
  ...
  const [image, setImage] = useState(profile[0].user_image_url);
  const [userProfile, setUserprofile] = useState('');
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
  const [getDate, setGetDate] = useState(profile[0].user_team_detail.birthdate);
  ...
  useEffect(() => {
    if (profile[0].user_team_detail.shoots === 'left') {
      setClick(true);
      setUnClick(false);
      setChecked('left');
    } else if (profile[0].user_team_detail.shoots === 'right') {
      setClick(false);
      setUnClick(true);
      setChecked('right');
    }
